...
class TestMockUserGrid(unittest.TestCase):
    ...
    def test_basic_get_multiple(self):
        ...
        self.ug.add_response("/channels", data=[self.channels[chid1], self.channels[chid2]])
    ...
    def add_channel(self, chid, text):
        ...
        channel = { 'name': text, 'uuid': chid, 'created': now, 'modified': now,
            'description': "{0} Description".format(text), 'title': '{0} Title'.format(text)
            , 'type': 'channels' }
    ...
    def add_story(self, storyid, text, chid=None):
        now = int(time.time())*1000
        story = { 'audio': 'unimplemented', 'audiointro': 'unimplemented',
                'created': now, 'description': '{0} Description'.format(text),
                'descriptor': '{0} Descriptor'.format(text), 'duration': 180,
                'hls': {
                    'playlist': 'unimplemented',
                    'processed_at': now,
                    'status': 'complete',
                    'status_update': now
                }, 'image': 'unimplemented', 'image_cdn': None, 'intro_hls': None,
                'metadata': {
                    'connecting': {
                        'contains': '/stories/{0}/connecting/contains'.format(storyid),
                        'owns': 'unimplemented'
                    },
                    'connections': {
                        'has': 'unimplemented', 'hasintro': 'unimplemented'
                    }
                },
                'modified': now, 'name': '{0} Name'.format(text), 'owner': 'unimplemented',
                'relatedStories': {
                    'global': [],
                    'last_processed': now
                },
                'status': { 'audio': 'complete', 'audioIntro': 'complete', 'picture': 'complete' },
                'tags': [],
                'title': '{0} Title'.format(text), 'type': 'stories', 'uuid': storyid }

        if chid:
            story['channels'] = ["{0}".format(chid)]
            story['channel_feeds'] = { chid: 0 }
